...
void menu(int OUT[ROWS][COLUMNS],int u[ROWS][COLUMNS]) {
    ...
    while (1) {
        ...
        printf("Insert the coordinates in the way (x y): ");
        scanf("%d %d", &j, &i);
...
int main(){
    int OUT[ROWS][COLUMNS] = { 0 };        // Save matrix
    int u[ROWS][COLUMNS] = { 0 };          
    init(OUT);             			       // Start matrix
    menu(OUT, u);            		       // Menu
    return 0;
}


//Based on Brain Sotelo and Ricardo Glez work
